...
export function resolveAssetsDir(absProject, assetsDirRel, configRel = "wrangler config") {
  ...
  if (!existsSync(assetsDir)) {
    throw new Error(`${configRel} assets.directory "${assetsDirRel}" not found`);
  }
  const dirStat = lstatSync(assetsDir);
  if (dirStat.isSymbolicLink()) {
    throw new Error(
      `${configRel} assets.directory "${assetsDirRel}" must not be a symlink`
    );
  }
  if (!dirStat.isDirectory()) {
    throw new Error(`${configRel} assets.directory "${assetsDirRel}" is not a directory`);
  }
  const projectReal = realpathSync(absProject);
  const assetsReal = realpathSync(assetsDir);
  const rel = path.relative(projectReal, assetsReal);
  if (rel === ".." || rel.startsWith(".." + path.sep) || path.isAbsolute(rel)) {
    throw new Error(
      `${configRel} assets.directory "${assetsDirRel}" resolves outside the project root`
    );
  }
  return assetsDir;
}
...
export function collectAssets(dir, { onIgnore = null } = {}) {
  ...
  const userPatterns = existsSync(ignoreFile)
    ? parseAssetIgnorePatterns(readFileSync(ignoreFile, "utf8"))
    : [];
  ...
  (function walk(cur, rel) {
    for (const entry of readdirSync(cur)) {
      ...
      if (st.isSymbolicLink()) {
        throw new Error(
          `assets: symlink not allowed at "${relPath}" ` +
          `(add "${relPath}" to .assetsignore to skip it; patterns ending in "/" match only real directories, not symlinks)`
        );
      }
      if (st.isDirectory()) {
        walk(full, relPath);
      ...
      if (!st.isFile()) {
        throw new Error(`assets: unsupported entry type at "${relPath}"`);
      }
      const realFull = realpathSync(full);
      const inside = path.relative(rootReal, realFull);
      if (inside === ".." || inside.startsWith(".." + path.sep) || path.isAbsolute(inside)) {
        throw new Error(`assets: "${relPath}" resolves outside the assets root`);
      }
      if (st.size > MAX_ASSET_FILE_BYTES) {
        throw new Error(
          `assets: "${relPath}" is ${st.size} bytes, exceeds ${MAX_ASSET_FILE_BYTES} per-file cap`
        );
      }
      totalBytes += st.size;
      ...
      out[relPath] = readFileSync(full).toString("base64");
    }
  })(dir, "");
  return out;
}
...
function compileAssetIgnoreRule(pattern) {
  ...
  try {
    ...
    throw new Error(`assets: invalid .assetsignore pattern ${JSON.stringify(originalPattern)}: ${message}`, {
      cause: err,
    });
  }
  return {
