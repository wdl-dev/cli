...
import {
  ...
  formatAiEnvNonInheritanceWarning,
...
export async function packWranglerProject({
  ...
  const aiEnvWarning = formatAiEnvNonInheritanceWarning(rawCfg, envName, configRel);
  // Verbose mode inherits Wrangler stderr and already shows its native warning.
  if (aiEnvWarning && !verbose) stderr(`warning: ${aiEnvWarning}`);
  ...
  const ai = wrapCli(() => parseAiBindingFromCfg(cfg, configRel));
  if (ai) {
    claimBinding(ai.binding);
    bindings[ai.binding] = { type: "ai" };
  }
  ...
  const { routes, workersDev, sessionPolicy } = wrapCli(() => {
    const routes = collectRoutes(cfg, configRel);
    return {
      routes,
      workersDev: parseWorkersDev(cfg, routes, configRel),
      sessionPolicy: parseSessionPolicy(cfg, configRel),
    };
  });
  ...
  try {
    ...
    const wranglerOpts = {
      cwd: absProject,
      // Older Wrangler releases offer skills installation on interactive stdin.
      // Verbose mode forwards output, but bundling stays non-interactive.
