...
export function validateUnsupportedWranglerConfig(rawCfg, envName, configRel = "wrangler config") {
  const selectedEnvCfg =
    envName &&
    rawCfg?.env &&
    typeof rawCfg.env === "object" &&
    !Array.isArray(rawCfg.env)
      ? rawCfg.env[envName]
      : null;

  for (const key of UNSUPPORTED_WRANGLER_KEYS) {
    if (hasConfiguredValue(rawCfg?.[key])) {
      throw new Error(
  ...
  if (hasConfiguredValue(rawCfg?.allowed_callers)) {
...
export function resolveWranglerConfig(rawCfg, envName, configRel = "wrangler config") {
  ...
  const availableEnvs = listNamedEnvironments(rawCfg);
  if (!envName) {
    ...
    return { cfg: rawCfg, envName: null };
  }
  ...
  if (!Object.hasOwn(rawCfg.env, envName)) {
    throw new Error(
  ...
  const envCfg = rawCfg.env[envName];
...
function listNamedEnvironments(cfg) {
  if (!cfg?.env || typeof cfg.env !== "object" || Array.isArray(cfg.env)) return [];
