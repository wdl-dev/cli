...
Control-plane D1 lifecycle API 对外返回 5xx 时会把消息收敛为 `Internal error`，只保留有界的 machine classifier：`upstreamCode`、`upstreamCategory`、`upstreamRetryable` 和 `upstreamStatus`。对外 4xx SQL / migration failure 仍可保留可操作的 D1 诊断。
