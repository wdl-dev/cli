 ...
 * handler's `values` type can't drift. Each entry is keyed by preset name; its
 * value lists the parsed `values` keys that preset contributes — which need not
 * match the name (the "control" preset contributes "control-url", "token", and
 * "no-token-store", not a "control" key).
