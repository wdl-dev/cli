...
export function formatD1List(body) {
  ...
  return databases.map((db) => `${db.databaseId}\tname=${db.databaseName || "-"}\tcreated=${db.createdAt || "-"}`);
}
...
export function formatD1MigrationList(body) {
  ...
  return migrations.map(
    (migration) => `${migration.id}\tapplied=${migration.appliedAt || "-"}\tchecksum=${migration.checksum || "-"}`
  );
}
...
export function formatD1MigrationStatus(body) {
  ...
  return migrations.map(
    (migration) => `${migration.id}\tstate=${migration.state}\tapplied=${migration.appliedAt || "-"}`
  );
}
...
export function formatD1MigrationApply(body) {
  ...
  return [
    ...applied.map((migration) => `Applied ${migration.id}\tstatements=${migration.statementCount ?? "-"}`),
