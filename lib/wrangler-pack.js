...
export async function packWranglerProject({
  ...
  const assetsCfg =
    cfg.assets && typeof cfg.assets === "object" && !Array.isArray(cfg.assets)
      ? /** @type {Record<string, unknown>} */ (cfg.assets)
      : null;
  const assetsDirRel = assetsCfg ? assetsCfg.directory : undefined;
  if (assetsDirRel) {
    const assetsDir = wrapCli(() =>
      // assets.directory is a config path; resolveAssetsDir validates it on disk.
      resolveAssetsDir(absProject, /** @type {string} */ (assetsDirRel), configRel)
    );
    /** @type {string[]} */
    const skippedAssets = [];
