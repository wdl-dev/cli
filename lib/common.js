...
export function loadCliControlEnv(env, {
  ...
  if (!ns) {
    ...
    const def = typeof s.defaultNs === "string" ? s.defaultNs : null;
  ...
  // slot that is still empty AND not supplied by a flag (resolved later). That
  // keeps the store unread when the credentials are already covered.
  if (ns) {
...
// (e.g. a token). Mirrors quoteValue in lib/token-store.js.
