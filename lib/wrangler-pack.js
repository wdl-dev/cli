...
import { hasOwn, manifestMap } from "./wrangler/utils.js";
...
export async function packWranglerProject({
  ...
  writeFileSync(tmpConfigPath, JSON.stringify({ ...rawCfg, name: "wdl-bundle-tmp" }), {
    flag: "wx",
  });
  try {
    const wranglerArgs = [
  ...
  if (cfg.allowed_callers != null) {
    if (!Array.isArray(cfg.allowed_callers)) {
      throw new CliError(`${configRel}: allowed_callers must be an array of strings`);
    }
    for (const entry of cfg.allowed_callers) {
      if (typeof entry !== "string" || (entry !== "*" && !NS_RE.test(entry))) {
        throw new CliError(`${configRel}: allowed_callers entries must be "*" or match ${NS_PATTERN}, got ${JSON.stringify(entry)}`);
      }
    }
    manifest.allowedCallers = [...cfg.allowed_callers];
  }
  ...
  const assetsDirRel = cfg.assets && cfg.assets.directory;
  if (assetsDirRel) {
    const assetsDir = wrapCli(() =>
      resolveAssetsDir(absProject, assetsDirRel, configRel)
    );
    const skippedAssets = [];
...
function collectRoutes(cfg, configRel) {
  const collected = [];
  const pushEntry = (r, source) => {
    if (typeof r === "string") collected.push(r);
    else if (r && typeof r === "object" && typeof r.pattern === "string") collected.push(r.pattern);
