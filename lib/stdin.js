// Reading from stdin/TTY: confirmation, visible single/multi-line prompts,
// raw-mode hidden input, and the secret reader shared by token/secret commands.
 ...
 * @property {(encoding: BufferEncoding) => unknown} setEncoding
 * @property {(mode: boolean) => unknown} [setRawMode]
 * @property {<A extends unknown[]>(event: string, listener: (...args: A) => void) => unknown} on
 * @property {<A extends unknown[]>(event: string, listener: (...args: A) => void) => unknown} off
...
export async function confirmAction({
  ...
  if (!stdin.isTTY) {
    throw new CliError(
      `Refusing to ${escapeTerminalText(action)} without interactive confirmation. Pass --yes to confirm.`
    );
  }
...
export function readTtyLine(stdin, { prompt, stderr, hidden = false } = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (wantHidden) {
      const failClosed = () => reject(new CliError("cannot hide input on this terminal; pipe the value in instead"));
...
/**
 * Read several visible TTY answers through one listener. A single reader keeps
 * pasted multi-line answers buffered between prompts; repeated readTtyLine()
 * calls would pause the stream and discard data after the first newline.
 * @param {StdinLike} stdin
 * @param {{ prompts: Array<string | ((answers: readonly string[]) => string)>, stderr?: (text: string) => void }} options
 * @returns {Promise<string[]>}
 */
export function readTtyLines(stdin, { prompts, stderr }) {
  if (!stdin.isTTY) throw new CliError("interactive input requires a TTY");
  ...
  return new Promise((resolve, reject) => {
    ...
    const writePrompt = () => {
      ...
      if (stderr) stderr(escapeTerminalText(typeof prompt === "function" ? prompt(answers) : prompt));
    };
    const submit = () => {
    ...
    const onData = (chunk) => {
      for (const ch of chunk) {
        ...
        if (ch === "\u0003") return fail(new CliError("input aborted"));
        if (ch === "\r") {
          skipLeadingLf = true;
          if (submit()) return;
          continue;
    ...
    const onEnd = () => {
      if (current || answers.length + 1 === prompts.length) answers.push(current);
      if (answers.length === prompts.length) finish(answers);
      else fail(new CliError("input ended before all answers were provided"));
    };
    /** @param {unknown} err */
    const onError = (err) => fail(err);
...
export function readSecretStdin(stdin, { prompt, stderr } = {}) {
  ...
  return new Promise((resolve, reject) => {
    let data = "";
    stdin.setEncoding("utf8");
    stdin.on("data", (/** @type {string} */ chunk) => (data += chunk));
