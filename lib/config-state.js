...
/**
 * @param {ConfigStateOptions} [options]
 */
export function resolveCliConfigState({
  values = {},
  env = process.env,
  cwd = process.cwd(),
  dotenvPath = ".env",
...
/**
 * Resolve as much diagnostic state as possible when the token store is
 * unreadable or malformed. Operating commands still fail closed in the normal
 * resolver; only diagnostics use this fallback.
 * @param {ConfigStateOptions} [options]
 */
export function resolveDiagnosticConfigState(options = {}) {
  const shownWarnings = new Set();
  const warn = options.warn;
  /** @param {string} line */
  const warnOnce = (line) => {
    if (shownWarnings.has(line)) return;
    shownWarnings.add(line);
    warn?.(line);
  };
  const diagnosticOptions = warn
    ? {
        ...options,
        warn: warnOnce,
      }
    : options;
  const requiredDiagnosticOptions = { ...diagnosticOptions, requireNamespace: true };
  try {
    return { state: resolveCliConfigState(requiredDiagnosticOptions), tokenStoreError: null };
  } catch (err) {
    if (!(err instanceof TokenStoreConfigError)) throw err;
    return {
      state: resolveCliConfigState({ ...requiredDiagnosticOptions, readStore: () => ({}) }),
