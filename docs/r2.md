...
Wrangler's `local_dev.experimental_s3_credentials` setting exposes an R2 bucket
through a local S3-compatible endpoint. WDL deploy does not run that local
service and rejects the field instead of silently dropping the credentials.
...
use canonical IMF-fixdate syntax, such as `Wed, 21 Oct 2015 07:28:00 GMT`.
Malformed values are rejected before the host binding call.
...
intend to stream bytes; on an interactive terminal, pass `--out <path>`. `--out`
accepts an explicit path outside the project and currently uses normal overwrite
semantics: an existing file is replaced, and a symlink target is followed.
