...
/** @returns {CliError} */
export function missingNamespaceError() {
  return new CliError("Missing namespace. Pass --ns <namespace> or set WDL_NS.");
}
...
/**
 * Reject arguments without repeating values from a command that may receive
 * credentials elsewhere in its grammar.
 * @param {string} label
 * @param {string} [recovery]
 * @returns {CliError}
 */
export function redactedArgumentError(label, recovery = "use --help for usage") {
  return new CliError(
    `${escapeTerminalText(label)} received invalid arguments; argument details were redacted; ${escapeTerminalText(recovery)}`
  );
}

/**
 * Reject malformed arguments to a command that accepts sensitive input.
 * @param {string} label
 * @returns {CliError}
 */
export function sensitiveInputArgumentError(label) {
  return new CliError(
    `${escapeTerminalText(label)} received invalid arguments; provide sensitive input through the prompt or stdin`
  );
}
...
/**
 * Normalize the page-size contract shared by paginated commands.
 * @param {string | undefined} value
 * @param {string} label
 * @returns {string | undefined}
 */
export function normalizePageLimit(value, label) {
  if (value == null || value === "") return undefined;
  const limit = Number(value);
  if (!Number.isInteger(limit) || limit < 1 || limit > 1000) {
    throw new CliError(`${escapeTerminalText(label)} must be an integer in [1, 1000]`);
  }
  return String(limit);
}
 ...
 */
export function commonCliOptions({
  namespace = true,
  controlUrl = true,
  token = true,
  noTokenStore = true,
 ...
 * @param {{ namespace?: boolean, controlUrl?: boolean, token?: boolean, noTokenStore?: boolean, json?: boolean, help?: boolean }} [options]
 */
export function commonCliOptionSpecs({
  namespace = true,
  controlUrl = true,
  token = true,
  noTokenStore = true,
...
const OPTION_DEFS = {
  ...
  controlUrl: defineCliOption(
    "control-url",
    { type: "string" },
    "--control-url <url>",
    "Control URL (env: CONTROL_URL)."
  ),
  ...
  noTokenStore: defineCliOption(
    "no-token-store",
    { type: "boolean" },
    "--no-token-store",
    "Ignore the global token store; resolve credentials from flag/env/.env only (env: WDL_TOKEN_STORE=off)."
  ),
...
/**
 * Read a JSON response while allowing command-specific guidance for a control
 * error code. Error JSON is parsed once and reused by the standard formatter.
 * @param {import("./control-fetch.js").ControlJsonResponse} res
 * @param {string} label
 * @param {(error: unknown) => string} errorHint
 * @returns {Promise<unknown>}
 */
export async function readJsonOrFailWithHint(res, label, errorHint) {
  ...
  try {
    ...
    throw new CliError(`${label} failed: ${formatHttpError(res.status, text, res.headers)}${errorHint(undefined)}`);
  }
  const error =
    body && typeof body === "object" && !Array.isArray(body)
      ? /** @type {{ error?: unknown }} */ (body).error
      : undefined;
  throw new CliError(
    `${label} failed: ${formatParsedHttpError(res.status, body, raw, res.headers ?? {})}${errorHint(error)}`
  );
}
...
const ERROR_SUMMARY_KEYS = new Set(["error", "message", "detail", "details", "stack"]);

const ARRAY_CONTEXT_KEYS = new Set(["blockers", "referrers", "reserved_modules", "warnings"]);
...
export function formatHttpError(status, text, headers = {}) {
  ...
  return formatParsedHttpError(status, body, raw, headers);
}

/**
 * Format an HTTP error body that the caller has already parsed as JSON.
 * @param {number | undefined} status
 * @param {unknown} body
 * @param {import("node:http").IncomingHttpHeaders} [headers]
 */
export function formatHttpErrorBody(status, body, headers = {}) {
  let raw;
  try {
    raw = JSON.stringify(body) ?? String(body);
  } catch {
    raw = String(body);
  }
  return formatParsedHttpError(status, body, raw, headers);
 ...
 * @param {import("node:http").IncomingHttpHeaders} headers
 */
function formatParsedHttpError(status, body, raw, headers) {
  const location = redirectLocation(status, headers);
