...
/**
 * Commands marked `sensitiveInput` redact every parser error. After parsing,
 * they also reject a separate string-option value that looks like a displaced
 * command word before another valid command path; explicit `--flag=value` and
 * ordinary flags-before-command calls remain unambiguous.
 * @param {{
 *   name: string,
 *   summary: string,
 *   options?: Array<import("./common.js").OptionListItem>,
 *   sensitiveInput?: { commandPaths: string[][] },
 *   defaults?: Record<string, unknown>,
 *   autoloadEnv?: boolean | ((positionals: string[]) => boolean),
 *   usage: () => string,
 *   run(ctx: { values: Record<string, unknown>, positionals: string[], context: CommandContext }): Promise<unknown> | unknown,
 * }} spec
 * @returns {{ main: (argv?: string[]) => Promise<void>, run: (argv?: string[], deps?: object) => Promise<unknown>, meta: { name: string, summary: string, autoloadEnv: boolean | ((positionals: string[]) => boolean), parseOptions: import("node:util").ParseArgsOptionsConfig } }}
 */
export function defineCommand(spec) {
  const { name, summary, options = [], sensitiveInput, defaults = {}, autoloadEnv = true, usage, run } = spec;
  ...
  async function runCommand(argv = process.argv.slice(2), deps = {}) {
    const { values, positionals, tokens } = (() => {
      try {
        ...
        if (sensitiveInput) {
          throw redactedArgumentError(name);
        }
        const message = err instanceof Error && err.message ? err.message : String(err);
        throw new CliError(escapeTerminalText(message));
      }
    })();
    const helpRequested = values.help || isHelpAlias(positionals);
    if (
      sensitiveInput &&
      !helpRequested &&
      hasAmbiguousSensitiveCommandPath(positionals, tokens, sensitiveInput.commandPaths)
    ) {
      throw redactedArgumentError(name, "put the command before its options or use --flag=value");
    }

    const context = buildContext(deps, defaults, values, positionals);

    if (printHelpIfRequested(helpRequested, usage, context.stdout)) return undefined;
 ...
 * @param {Array<
 *   | { kind: "option", index: number, value?: string, inlineValue?: boolean }
 *   | { kind: "positional", index: number, value: string }
 *   | { kind: "option-terminator", index: number }
 * >} tokens
 * @param {string[][]} commandPaths
 */
function hasAmbiguousSensitiveCommandPath(positionals, tokens, commandPaths) {
  let pathLength = 0;
  for (const path of commandPaths) {
    if (path.every((segment, index) => positionals[index] === segment)) {
      pathLength = Math.max(pathLength, path.length);
    }
  }
  if (pathLength === 0) return false;

  const positionalTokens = tokens.filter((token) => token.kind === "positional");
  const commandEndIndex = positionalTokens[pathLength - 1]?.index;
  if (commandEndIndex === undefined) return false;
  const commandWords = new Set(commandPaths.flat());
  return tokens.some(
    (token) =>
      token.kind === "option" &&
      token.index < commandEndIndex &&
      token.inlineValue === false &&
      typeof token.value === "string" &&
      commandWords.has(token.value)
  );
}
...
function buildContext(deps, commandDefaults, values, positionals) {
  ...
  // Fail-fast on an unresolved namespace: callers reject a missing namespace
  // first, so this only fires if a command forgets that check —
  ...
  context.nsUrl = (/** @type {string[]} */ ...segments) => {
    ...
    return segments.length === 0 ? base : `${base}/${segments.map((s) => encodePath(s)).join("/")}`;
  };
