...
export function validateControlHeaders(headers) {
  ...
  for (const [name, value] of Object.entries(headers)) {
    try {
      ...
      throw new CliError(
        `control request failed: invalid HTTP header ${formatDiagnosticValue(name)}: ${escapeTerminalText(message)}`
      );
    }
  }
}
...
export function controlRequestError(err) {
  ...
  const code = typeof codeValue === "string" ? `${escapeTerminalText(codeValue)} ` : "";
...
function streamControlResponse(res, body) {
  ...
  return {
    ...
    arrayBuffer: async () =>
      (
        await readControlResponse(body, {
