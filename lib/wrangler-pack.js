...
export async function packWranglerProject({
  ...
  for (const name of Object.keys(vars)) claimBinding(name);

  if (Array.isArray(cfg.compatibility_flags) && cfg.compatibility_flags.includes("experimental")) {
    throw new CliError(
      `${configRel}: compatibility_flags "experimental" is not supported by WDL`
    );
  }
...
export function installTempFileCleanup(
  ...
  let active = true;
  const cleanup = () => rmSync(filePath, { force: true });
