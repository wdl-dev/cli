...
| `binding name collision: <NAME>` | `[vars]`、显式 bindings 或隐式 `ASSETS` binding 复用了 runtime env 名称；重命名其中一个。 |
| `experimental_compat_flag_unsupported` | 移除 experimental workerd compatibility flag。 |
| `python_workers_unsupported` | WDL 不支持 Python Workers；移除 Python Worker modules。CLI 也会对本地 `.py` module fail-fast。 |
