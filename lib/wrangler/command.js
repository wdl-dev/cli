...
export function checkWranglerVersion({ execFile = execFileSync, cwd, env, wrangler }) {
  const result = probeWranglerVersion({ execFile, cwd, env, wrangler });
  if (result.major < MIN_WRANGLER_MAJOR) {
    throw new CliError(
      `wdl deploy requires Wrangler v${MIN_WRANGLER_MAJOR} (wrangler@^${MIN_WRANGLER_MAJOR}); ` +
        `found v${result.major}. Upgrade the Worker project's wrangler dependency before deploying.`
    );
  }
  return result;
}
