...
import { CliError, parseDotEnvSection, parseDotEnvValue } from "./common.js";
...
export function writeTokenStore(storePath, store) {
  ...
  if (store.defaultNs && namespaces[store.defaultNs]) {
  ...
  // writeFileSync's mode only applies on create; force perms on an existing file.
  chmodSync(storePath, 0o600);
}

// Escape for the double-quoted dialect: backslash first, then the rest, so a
// value with quotes/newlines/tabs survives a read→write→read round trip.
...
export const __test__ = { quoteValue, STORE_KEYS };
