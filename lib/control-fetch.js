...
export function validateControlHeaders(headers) {
  ...
  for (const [name, value] of Object.entries(headers)) {
    try {
      ...
      throw new CliError(`control request failed: invalid HTTP header ${JSON.stringify(name)}: ${message}`);
    }
  }
}
...
export function controlRequestOptions(u, env = process.env) {
  ...
  const opts = {
    ...
    headers: {
      Host: u.host,
      "User-Agent": `wdl-cli/${currentCliVersion() || "unknown"}`,
    },
    agent: false,
  };
...
function controlConnectTarget(u, isHttps, env) {
  ...
  return parseConnectHost(override, defaultPort);
}
 ...
 * @returns {{ host: string, port: number }}
 */
function parseConnectHost(raw, defaultPort) {
  const text = raw.trim();
  if (/^[a-z][a-z\d+.-]*:\/\//i.test(text)) {
    try {
      const parsed = new URL(text);
      return { host: bareUrlHostname(parsed), port: validPort(parsed.port || defaultPort, raw) };
    } catch (err) {
      if (err instanceof CliError) throw err;
      throw new CliError(`Invalid CONTROL_CONNECT_HOST ${JSON.stringify(raw)}.`);
    }
  }
...
function validPort(value, raw) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new CliError(`Invalid CONTROL_CONNECT_HOST ${JSON.stringify(raw)}: port must be in [1, 65535]`);
  }
  return port;
}

/** @param {URL} u */
function bareUrlHostname(u) {
