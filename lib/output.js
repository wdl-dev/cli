...
export function formatDiagnosticValue(value) {
  // JSON.stringify renders NaN and ±Infinity as "null", hiding what was written.
  if (typeof value === "number" && !Number.isFinite(value)) return escapeTerminalText(String(value));
  // TOML datetimes parse into Date; quoting their ISO form would read as a string.
  if (value instanceof Date) return escapeTerminalText(`datetime ${value.toISOString()}`);
