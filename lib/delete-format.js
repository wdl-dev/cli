...
const ASSET_WARNING_KEYS = ["code", "message", "path", "key", "prefix", "reason"];
 ...
 * @property {string | null} [activeDeleted]
...
export function formatWorkerDelete(body) {
  ...
  const versions =
    Array.isArray(body.versionsDeleted) && body.versionsDeleted.length
      ? body.versionsDeleted.map((version) => field(version)).join(",")
      : "-";
  const active = field(body.activeDeleted);
  const lines = [`OK ${field(body.namespace)}/${field(body.name)} deleted active=${active} versions=${versions}`];
...
function formatDryRun(body) {
  ...
  if (body.hasWorkerSecrets) lines.push("  worker secrets present");
  if (body.hasWorkflowDefs) lines.push("  workflow definitions present");
  if (Array.isArray(body.affectedHosts) && body.affectedHosts.length) {
    lines.push(`  affected hosts: ${body.affectedHosts.map((host) => field(host)).join(",")}`);
  }
  if (Number.isFinite(body.queueConsumersRemoved) && Number(body.queueConsumersRemoved) > 0) {
...
function appendBlockers(lines, blockers) {
  ...
  for (const blocker of blockers) {
    ...
    if (Number.isFinite(blocker.crossNamespaceReferrerCount) && Number(blocker.crossNamespaceReferrerCount) > 0) {
...
function appendWorkflowBlocker(lines, blocker) {
  ...
  for (const ref of refs) {
    lines.push(`    ${field(ref.workflowKey)} instance=${field(ref.instanceId)}`);
  }
}
