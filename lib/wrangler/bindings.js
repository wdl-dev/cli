...
const MAX_QUEUE_CONSUMER_BATCH_TIMEOUT_SECONDS = 60;
    ...
    for (const rawConsumer of queuesTable.consumers) {
      ...
      if (c.max_batch_timeout != null) {
        entry.maxBatchTimeoutMs = normalizeQueueDelayConfig(
          ...
          MAX_QUEUE_CONSUMER_BATCH_TIMEOUT_SECONDS
        ) * 1000;
      }
 ...
 * @returns {number}
 */
function normalizeQueueDelayConfig(value, configRel, field, maxSeconds = MAX_QUEUE_DELAY_SECONDS) {
  if (typeof value !== "number" || !Number.isInteger(value) || value < 0 || value > maxSeconds) {
    throw new Error(`${configRel}: ${field} must be an integer in [0, ${maxSeconds}]`);
  }
  return value;
}
