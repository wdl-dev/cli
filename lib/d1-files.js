...
export function readMigrationFiles(dir = "migrations") {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      throw new CliError(
        `d1 migrations: ${escapeTerminalText(entry.name)} is a symlink; copy the SQL file into the migrations dir`
      );
    }
...
function readMigrationFile(root, filename) {
  ...
  try {
    ...
    if (st.isSymbolicLink()) {
      throw new CliError(
        `d1 migrations: ${escapeTerminalText(filename)} is a symlink; copy the SQL file into the migrations dir`
      );
    }
