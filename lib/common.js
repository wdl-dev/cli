...
export async function readJsonOrFail(res, label) {
  ...
  // throwHttpErrorIfNotOk only returns for a 2xx response, which always carries
  // a json reader; the optional type is for the error-path callers above it.
...
function formatHttpError(status, text) {
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (ARRAY_CONTEXT_KEYS.has(key) && Array.isArray(value) && value.length > 0) {
      ...
      context.push(`${safeKey}=${escapeTerminalText(JSON.stringify(value))}`);
    }
  }
