...
| `control did not confirm session_policy = restart`                          | The control plane predates `[wdl] session_policy`                                                                   | The version was uploaded but not promoted; re-running before upgrading control cannot help, so upgrade control and deploy again |
| `control promoted the worker without confirming its restart session policy` | Control promoted but did not echo the policy or a usable restart sequence                                           | Upgrade control, then redeploy; the new version is live but its sessions may still be pinned to the old one                     |
