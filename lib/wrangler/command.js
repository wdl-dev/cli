...
export const SUPPORTED_WRANGLER_MAJOR = 4;
...
export function resolveWranglerCommand({
  ...
  // Keep deploy offline by default. `npx --yes wrangler@^4` may hit the
  ...
  if (env.WDL_ALLOW_NPX_WRANGLER === "1") {
    return npxWrangler(env, platform);
  }
...
export function checkWranglerVersion({ execFile = execFileSync, cwd, env, wrangler, fallbackVersion }) {
  const result = probeWranglerVersion({ execFile, cwd, env, wrangler, fallbackVersion });
  if (result.major !== SUPPORTED_WRANGLER_MAJOR) {
    throw new CliError(
      `wdl deploy requires Wrangler v${SUPPORTED_WRANGLER_MAJOR} (wrangler@^${SUPPORTED_WRANGLER_MAJOR}); ` +
        `found v${result.version}. Install a supported v${SUPPORTED_WRANGLER_MAJOR} release in the Worker project before deploying.`
    );
  }
  return result;
}
 ...
 * @returns {{ version: string, major: number }}
 */
export function probeWranglerVersion({ execFile = execFileSync, cwd, env, wrangler, fallbackVersion }) {
  let output;
  try {
  ...
  const version = parseWranglerVersion(output) || parseWranglerVersion(fallbackVersion?.() ?? "");
  if (version == null) {
    throw new CliError(
...
export function wranglerChildEnv(env) {
  ...
  const childEnv = {
    ...
    // WDL captures Wrangler's banner and only needs local bundle output. Hide
    // the banner (and its normal update check) and disable anonymous telemetry.
...
export function formatWranglerFailure(rawErr) {
  ...
  const output = [err.stdout, err.stderr].map(toText).filter(Boolean).join("\n").trim();
 ...
 * @returns {string | null}
 */
export function parseWranglerVersion(output) {
  const match = toText(output).match(/\b\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?\b/);
  return match?.[0] ?? null;
}
 ...
 * @param {NodeJS.Platform} platform
 */
function npxWrangler(env, platform) {
  const args = ["--yes", "wrangler@^4"];
  if (platform !== "win32") return { command: "npx", args, source: "npx" };

  // Node >= 20.12 refuses to execFile the npx.cmd shim. A real executable shim
  // (for example Volta's npx.exe) remains safe to execute directly; otherwise
  // locate npm's JS entry and run it with Node.
  for (const dir of (env.PATH || "").split(path.delimiter)) {
    if (!dir) continue;
    const executable = path.join(dir, "npx.exe");
    if (existsSync(executable)) return { command: executable, args, source: "npx" };
  }

  const candidates = [];
  if (env.npm_execpath) candidates.push(path.join(path.dirname(env.npm_execpath), "npx-cli.js"));
  for (const dir of (env.PATH || "").split(path.delimiter)) {
    if (dir) candidates.push(path.join(dir, "node_modules", "npm", "bin", "npx-cli.js"));
  }
  const script = candidates.find((candidate) => existsSync(candidate));
  if (script) return { command: process.execPath, args: [script, ...args], source: "npx" };

  throw new CliError(
    "WDL_ALLOW_NPX_WRANGLER=1 is set, but no runnable npx installation was found. " +
      "Install wrangler@^4 in the Worker project or set WDL_WRANGLER_BIN to a runnable Wrangler entry."
  );
}
