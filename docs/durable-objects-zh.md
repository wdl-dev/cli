...
Promotion 与 route 变更在同一个事务中原子提交该策略；后续的 `preserve` promotion 可以覆盖尚未被观察到的 restart 工作，但无法撤销已经发生的关闭或 facet abort。deploy 失败会留下什么、以及如何恢复，见 [deploy-zh.md](./deploy-zh.md#常见错误)。
