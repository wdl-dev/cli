...
export function parseTriggers(triggers, configRel = "wrangler config") {
  ...
  const allowedKeys = new Set(["crons", "schedules"]);
  const unsupportedKeys = Object.keys(triggersTable).filter((key) => !allowedKeys.has(key));
  if (unsupportedKeys.length > 0) {
    throw new Error(
      `${configRel}: [triggers] contains unsupported field(s): ${formatConfigKeyList(unsupportedKeys)}; ` +
        "WDL supports only crons and schedules"
    );
  }
  ...
  if (queuesTable.consumers != null) {
    ...
    for (const rawConsumer of queuesTable.consumers) {
      ...
      if (c.max_batch_timeout != null) {
        entry.maxBatchTimeoutMs =
          normalizeQueueDelayConfig(
            c.max_batch_timeout,
            configRel,
...
export function parseKvNamespacesFromCfg(cfg, configRel = "wrangler config") {
  ...
  for (const rawEntry of cfg.kv_namespaces) {
    ...
    if (unknownKeys.length > 0) {
      throw new Error(`${configRel}: [[kv_namespaces]] contains unknown field(s): ${formatConfigKeyList(unknownKeys)}`);
    }
    ...
    if (typeof entry.id !== "string" || !entry.id.trim()) {
      throw new Error(
        `${configRel}: [[kv_namespaces]] ${formatConfigLabel(entry.binding)}: 'id' must be a non-empty string`
      );
    }
...
export function parseD1DatabasesFromCfg(cfg, configRel = "wrangler config") {
  ...
  for (const rawEntry of cfg.d1_databases) {
    ...
    const databaseRef =
      typeof entry.database_id === "string" && entry.database_id.trim()
        ? entry.database_id.trim()
        : entry.database_name;
    if (typeof databaseRef !== "string" || !databaseRef.trim()) {
      throw new Error(
        `${configRel}: [[d1_databases]] ${formatConfigLabel(entry.binding)}: database_name or database_id is required`
      );
    }
 ...
 * @param {WranglerConfig} cfg
 * @param {string} [configRel]
 * @returns {{ binding: string } | null}
 */
export function parseAiBindingFromCfg(cfg, configRel = "wrangler config") {
  configRel = formatConfigRel(configRel);
  if (cfg.ai === undefined) return null;
  const ai = asRecord(cfg.ai);
  if (!ai) throw new Error(`${configRel}: [ai] must be a table`);
  const unknownKeys = Object.keys(ai).filter((key) => key !== "binding");
  if (unknownKeys.length > 0) {
    throw new Error(`${configRel}: [ai] contains unsupported field(s): ${formatConfigKeyList(unknownKeys)}`);
  }
  if (typeof ai.binding !== "string" || !ai.binding.trim()) {
    throw new Error(`${configRel}: [ai].binding is required`);
  }
  const binding = ai.binding.trim();
  assertNotRuntimeReservedBinding(configRel, "[ai]", binding);
  assertValidBindingName(configRel, "[ai]", binding);
  return { binding };
}
...
export function parseServicesFromCfg(cfg, configRel = "wrangler config") {
  ...
  for (const rawEntry of cfg.services) {
    ...
    if (typeof entry.binding !== "string" || !entry.binding.trim()) {
      throw new Error(
        `${configRel}: [[services]] binding must be a non-empty string, got ${formatDiagnosticValue(entry.binding)}`
      );
    }
    if (typeof entry.service !== "string" || !entry.service.trim()) {
      throw new Error(
        `${configRel}: [[services]] ${formatConfigLabel(entry.binding)}: service must be a non-empty string, got ${formatDiagnosticValue(entry.service)}`
      );
    }
...
export function parseDurableObjectsFromCfg(cfg, configRel = "wrangler config") {
  ...
  for (const rawEntry of bindingList) {
    ...
    if (entry.script_name != null) {
      throw new Error(
        `${configRel}: [[durable_objects.bindings]] ${formatConfigLabel(entry.name)}: script_name is not supported by WDL Durable Objects yet`
      );
    }
...
export function parseWorkflowsFromCfg(cfg, configRel = "wrangler config") {
  ...
  const allowedKeys = new Set(["name", "binding", "class_name"]);
  ...
  for (const rawEntry of cfg.workflows) {
    ...
    if (entry.script_name != null) {
      throw new Error(
        `${configRel}: [[workflows]] ${formatConfigLabel(entry.name)}: script_name is not supported by WDL Workflows`
      );
    }
    const unknownKeys = Object.keys(entry).filter((key) => !allowedKeys.has(key));
    if (unknownKeys.length > 0) {
      throw new Error(`${configRel}: [[workflows]] contains unknown field(s): ${formatConfigKeyList(unknownKeys)}`);
    }
    if (RESERVED_OBJECT_KEYS.has(entry.name)) {
      throw new Error(
        `${configRel}: [[workflows]] ${formatConfigLabel(entry.name)}: name is a reserved Object.prototype key`
      );
    }
    ...
    if (RESERVED_OBJECT_KEYS.has(entry.binding)) {
      throw new Error(
        `${configRel}: [[workflows]] ${formatConfigLabel(entry.name)}: binding is a reserved Object.prototype key`
      );
    }
...
export function parseExportsFromCfg(cfg, configRel = "wrangler config") {
  ...
  if (!Array.isArray(cfg.exports)) {
    throw new Error(
      `${configRel}: WDL [[exports]] must be an array of tables; ` +
