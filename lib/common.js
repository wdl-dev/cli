...
// The project's "set" predicate: a value counts as set only when it is a
// non-empty string. An empty string ("") or a non-string (undefined on an
// injected env object, a boolean/missing flag) is "absent" everywhere here — the
// .env load guard, the store gap-fill, flag detection, and namespace resolution
// all share this one rule, so the convention can never drift between sites.
