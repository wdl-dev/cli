...
export function resolveControlUrl(values, env = process.env) {
  ...
  const withScheme = /^[a-z][a-z\d+.-]*:\/\//i.test(text) ? text : `${defaultSchemeForBareControlUrl(text)}://${text}`;
  const normalized = withScheme.replace(/\/+$/, "");
  let parsed;
  try {
    parsed = new URL(normalized);
  } catch {
    throw new CliError("Invalid control URL.");
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new CliError("Invalid control URL: expected http:// or https://.");
  }
  if (parsed.username || parsed.password) {
    // Do not echo the raw URL: it may contain a password.
    throw new CliError("Invalid control URL: embedded usernames and passwords are not supported.");
  }
  // URL.search/hash are empty for a bare trailing `?` / `#`, even though
  // appending endpoint text would still put it in that URL component.
  if (/[?#]/.test(normalized)) {
    throw new CliError("Invalid control URL: query strings and fragments are not supported.");
  }
  return normalized;
}
...
export function resolveControlContext(values, env = process.env) {
  ...
  if (!token) {
    throw new CliError(
      "Missing admin token. Run 'wdl token set --ns <ns> --control-url <url>' (recommended), pass --token <tok>, or set ADMIN_TOKEN."
    );
  }
  return {
...
function insecureControlUrlReason(controlUrl, env) {
  ...
  if (!isLocalDevHost(actualHost)) {
    return (
      `control URL ${escapeTerminalText(controlUrl)} is plain http and ` +
...
// Loopback / reserved test hosts trusted by the plaintext-token warning. Bare
// URL scheme selection also has a legacy :8080 compatibility exception, but a
// port alone never makes a host safe for plaintext credentials. Accepts both
// the bare IPv6 form and the bracketed form URL.hostname produces.
/** @param {string} host */
 ...
 * @returns {string[]}
 */
export function loadCliDotEnv(env = process.env, path = ".env", options = {}) {
  let text;
  try {
 ...
 * }} [options]
 */
export function loadCliControlEnv(
  env,
  {
    dotenvPath,
    nsFromFlag,
  ...
  let storeData;
  const getStore = () => {
    if (storeData !== undefined) return storeData;
    storeData = readStore(env) || {};
    return storeData;
  };
  /** @type {unknown} */
  let storeReadError;
  ...
  } else if ((requireNamespace || needsFill) && storeReadError !== undefined) {
    // A failed default-namespace read is optional only when explicit
    // credentials cover a command that needs no namespace (for example,
    // `whoami`). Otherwise preserve its actionable permission/parse error
    // instead of replacing it with a missing namespace/URL error.
...
function guardCrossOriginControlEnv(env, loadedFromDotenv, tokenFromFlag, onCrossOrigin) {
  ...
  const tokenIsFromDotenv = loadedFromDotenv.has("ADMIN_TOKEN") && isNonEmptyString(env.ADMIN_TOKEN) && !tokenFromFlag;
