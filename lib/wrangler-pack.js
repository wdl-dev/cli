...
import { RESERVED_OBJECT_KEYS, WDL_RESERVED_BINDING_RE } from "./ns-pattern.js";
...
export { collectAssets, MAX_ASSET_FILE_BYTES, MAX_ASSETS_TOTAL_BYTES, resolveAssetsDir } from "./wrangler/assets.js";
export {
  parseD1DatabasesFromCfg,
  parseDurableObjectsFromCfg,
  parseExportsFromCfg,
...
export { parseWranglerMajorVersion, resolveWranglerCommand, wranglerChildEnv } from "./wrangler/command.js";
export {
  createWranglerBundleConfig,
  formatWranglerConfigShadowWarning,
  loadWranglerConfig,
 ...
 * @property {boolean} [workersDev]
 * @property {"restart"} [sessionPolicy]
...
export async function packWranglerProject({
  ...
  if (typeof cfg.name !== "string" || !cfg.name.trim()) {
    throw new CliError(`${shownConfig}: 'name' must be a non-empty string`);
  }
  ...
  const { producers: queueProducers, consumers: queueConsumers } = wrapCli(() => parseQueues(cfg.queues, configRel));
  ...
  const workersDev = parseWorkersDev(cfg, routes, configRel);
  const sessionPolicy = wrapCli(() => parseSessionPolicy(cfg, configRel));
  ...
  const wranglerBundleConfig = createWranglerBundleConfig(rawCfg);
  writeFileSync(tmpConfigPath, JSON.stringify(wranglerBundleConfig), {
    flag: "wx",
  });
  const cleanupTempConfig = installTempFileCleanup(tmpConfigPath);
  let bundlingFailed = false;
  try {
    const wranglerArgs = [...wrangler.args, "deploy", "--dry-run", `--outdir=${outDir}`, "--config", tmpConfigPath];
  ...
  if (!workersDev) manifest.workersDev = false;
  if (sessionPolicy === "restart") manifest.sessionPolicy = "restart";
...
export function collectRoutes(cfg, configRel) {
  ...
  if (cfg.route !== undefined && cfg.routes !== undefined) {
    throw new CliError(`${shownConfig}: specify either "route" or "routes", not both`);
  }
  ...
  if (cfg.routes !== undefined) {
    ...
    if (!Array.isArray(cfg.routes)) {
      throw new CliError(`${shownConfig}: "routes" must be an array of strings or { pattern } tables`);
    }
 ...
 * @returns {boolean}
 */
export function parseWorkersDev(cfg, routes, configRel) {
  if (cfg.workers_dev === undefined) return true;
  const shownConfig = escapeTerminalText(configRel);
  if (typeof cfg.workers_dev !== "boolean") {
    throw new CliError(`${shownConfig}: "workers_dev" must be a boolean`);
  }
  if (!cfg.workers_dev && routes.length === 0) {
    throw new CliError(`${shownConfig}: workers_dev = false requires at least one route pattern`);
  }
