...
export function tokenStoreDir(env = process.env, homedir = os.homedir, platform = process.platform) {
  ...
  const base = isNonEmptyString(env.XDG_CONFIG_HOME) ? env.XDG_CONFIG_HOME : path.join(homedir(), ".config");
...
export function readTokenStore(storePath) {
  ...
  let text;
  try {
    // POSIX O_NOFOLLOW closes the lstat/open race on the final path component.
    // Windows has no equivalent fs flag, so retain the static lstat rejection
    // there; fstat still validates the descriptor actually read on all hosts.
    const expectedStoreStat = process.platform === "win32" ? lstatSync(storePath) : null;
    if (expectedStoreStat) assertStoreFileSecure(storePath, expectedStoreStat);
    const flags = constants.O_RDONLY | (process.platform === "win32" ? 0 : constants.O_NOFOLLOW | constants.O_NONBLOCK);
    const fd = openSync(storePath, flags);
    try {
      const openedStoreStat = fstatSync(fd);
      assertStoreFileSecure(storePath, openedStoreStat);
      if (
        expectedStoreStat &&
        (expectedStoreStat.dev !== openedStoreStat.dev || expectedStoreStat.ino !== openedStoreStat.ino)
      ) {
        throw new CliError(`refusing to read credentials: ${escapeTerminalText(storePath)} changed while opening`);
      }
      assertStoreDirSecure(path.dirname(storePath), process.platform, "read");
    ...
    if (hasErrorCode(err) && err.code === "ELOOP") {
      throw new CliError(`refusing to read credentials: ${escapeTerminalText(storePath)} is not a regular file`);
    }
    throw wrapTokenStoreFsError(err, `failed to read credential store ${escapeTerminalText(storePath)}`);
  }

  try {
    return parseTokenStoreText(text);
  } catch (err) {
    if (!(err instanceof CliError)) throw err;
    throw new CliError(
      `failed to parse credential store ${escapeTerminalText(storePath)}: ${formatTokenStoreError(err)}`
    );
  }
}
 ...
 * @returns {{ defaultNs: string | null, namespaces: Record<string, Record<string, string>> }}
 */
function parseTokenStoreText(text) {
  ...
  for (const [idx, rawLine] of text
    ...
    if (nextSection !== null) {
      ...
      if (!Object.hasOwn(namespaces, section)) {
        Object.defineProperty(namespaces, section, {
          value: {},
          writable: true,
          enumerable: true,
          configurable: true,
 ...
 */
export function assertStoreDirSecure(storeDir, platform = process.platform, operation = "write") {
  const storeStat = statStoreDir(storeDir, operation);
  if (platform === "win32") return;
  if ((storeStat.mode & 0o022) !== 0) {
    const quotedDir = shellArgForDisplay(storeDir);
    throw new CliError(
      `refusing to ${operation} credentials: ${escapeTerminalText(storeDir)} is group/world-writable; ` +
        `restrict it with \`chmod 700 ${quotedDir}\``
    );
  }
}
 ...
 * @param {"read" | "write"} operation
 */
function statStoreDir(storeDir, operation) {
  // Follow a user-managed config-directory symlink (or Windows junction), but
  // validate the resolved target before using it. The credentials file itself
  // is independently opened and descriptor-checked, and may not be a symlink.
  const storeStat = statSync(storeDir);
  if (!storeStat.isDirectory()) {
    throw new CliError(`refusing to ${operation} credentials: ${escapeTerminalText(storeDir)} is not a directory`);
  }
  return storeStat;
}
...
/**
 * @param {string} storePath
 * @param {import("node:fs").Stats} storeStat
 * @param {NodeJS.Platform} [platform]
 */
function assertStoreFileSecure(storePath, storeStat, platform = process.platform) {
  const escapedPath = escapeTerminalText(storePath);
  if (!storeStat.isFile()) {
    throw new CliError(`refusing to read credentials: ${escapedPath} is not a regular file`);
  }
  if (platform !== "win32" && typeof process.geteuid === "function" && storeStat.uid !== process.geteuid()) {
    throw new CliError(`refusing to read credentials: ${escapedPath} is not owned by the current user`);
  }
  if (platform !== "win32" && (storeStat.mode & 0o077) !== 0) {
    const quotedPath = shellArgForDisplay(storePath);
    throw new CliError(
      `refusing to read credentials: ${escapedPath} is accessible by group/other users; restrict it with \`chmod 600 ${quotedPath}\``
    );
  }
}
...
function writeTokenStoreFile(storePath, store, { beforeCommit = () => {}, tempDir = path.dirname(storePath) } = {}) {
  ...
  const lines = ["# Managed by `wdl token`. Do not hand-edit — use a project .env for overrides.", ""];
...
export function updateTokenStore(
  storePath,
  update,
  { lockTimeoutMs = DEFAULT_LOCK_TIMEOUT_MS, staleLockMs = DEFAULT_STALE_LOCK_MS } = {}
) {
  /** @type {R | undefined} */
  let result;
...
function prepareTokenStoreDir(storeDir) {
  ...
  // Confirm the resolved path is a directory before chmod can affect it.
  statStoreDir(storeDir, "write");
...
function acquireTokenStoreLock(lockDir, lockTimeoutMs, staleLockMs) {
  ...
  while (true) {
    try {
      ...
      if (hasErrorCode(err) && err.code === "EEXIST" && recoverTokenStoreLock(lockDir, owner, staleLockMs)) {
        return owner;
      }
      if (Date.now() >= deadline) {
        throw new CliError("credential store is locked by another wdl token command; retry after it finishes");
      }
      Atomics.wait(LOCK_WAIT, 0, 0, LOCK_WAIT_MS);
    }
  }
...
function recoverTokenStoreLock(lockDir, owner, staleLockMs) {
  const recoveredPath = moveReleasedTokenStoreLock(lockDir) || moveStaleTokenStoreLock(lockDir, staleLockMs);
...
function isTokenStoreLockContentionError(err) {
  return hasErrorCode(err) && (err.code === "EEXIST" || err.code === LOCK_CONTENTION_CODE);
}
...
function isLockRecoveryFsError(err) {
  return hasErrorCode(err) && (err.code === "EACCES" || err.code === "EPERM" || err.code === "ENOTDIR");
}
